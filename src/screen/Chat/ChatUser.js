...
const initialThread = [
  {
    id: 'm1',
    text: 'Hi! Any tips for keeping my dog hydrated in summer?',
    fromMe: true,
    time: '11:40',
  },
  { id: 'm2', text: 'Absolutely! Fresh water in multiple spots helps.', fromMe: false, time: '11:41' },
  { id: 'm3', text: 'You can also add wet food to increase water intake.', fromMe: false, time: '11:42' },
  { id: 'm4', text: 'Noted, thank you! 🙌', fromMe: true, time: '11:43' },
];
...
const Bubble = ({ message }) => {
  const bubbleStyle = message.fromMe ? styles.bubbleMe : styles.bubbleOther;
  const textStyle = message.fromMe ? styles.bubbleTextMe : styles.bubbleTextOther;
  return (
...
const ChatUser = () => {
  ...
  useEffect(() => {
    if (route?.params?.chat) {
      setSelectedChat(route.params.chat);
    }
  }, [route?.params?.chat]);
    ...
    if (segment === 'Chats') {
      return (
        <FlatList
          ...
          renderItem={({ item }) => (
            <ChatRow item={item} onPress={() => setSelectedChat(item)} />
          )}
    ...
    return (
      ...
      >
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={[styles.bubbleRow, item.fromMe ? styles.rowEnd : styles.rowStart]}>
              {!item.fromMe && (
                <Image source={{ uri: selectedChat?.avatar || webAvatars[0] }} style={styles.threadAvatar} />
              )}
              <Bubble message={item} />
            </View>
          )}
  ...
  return (
    <Container backgroundColor={COLORS.white} statusBarBackgroundColor={COLORS.white}>
      <CustomHeader title={headerTitle} showBackButton={!!selectedChat} />
      {!selectedChat && (
        <>
